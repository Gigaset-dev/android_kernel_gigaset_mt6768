...
//prize added by huarui, fts support, 20190111-start
#if defined(CONFIG_PRIZE_HARDWARE_INFO)
#include "../../../misc/mediatek/hardware_info/hardware_info.h"
extern struct hardware_info current_tp_info;
#endif
//prize added by huarui, fts support, 20190111-end
/*****************************************************************************
Main control platform----Qualcomm
{
{MSM8940,Kernel 3.18,android 7.1.2},
{MSM8937,Kernel 3.18,android 7.1.2}
...
static int hyn_irq_registration(struct i2c_client *client)
{
    ...
    if (node)
    {
      ...
      //  hyn_ts_data->use_irq = irq_of_parse_and_map(node, 0);
       ...
       if (hyn_ts_data->use_irq != hyn_ts_data->client->irq)
        HYN_INFO("IRQs are inconsistent, please check <interrupts> & <hynitron,irq-gpio> in DTS");
...
int hyn_irq_init(struct i2c_client *client)
{
    ...
    HYN_INFO("irq in ts_data:%d irq in client:%d", hyn_ts_data->use_irq, hyn_ts_data->client->irq);
...
#if 0
static int hyn_pm_suspend(struct tp_device *dev)
{
	if(!hyn_ts_data){
		HYN_ERROR("pm_suspend data pointor is null,no the device \n");
		return -1;
...
static int hyn_pm_resume(struct tp_device *dev)
{
	if(!hyn_ts_data){
		HYN_ERROR("pm_resume data pointor is null,no the device \n");
		return -1;
...
static int hyn_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (IS_ERR(ts_data->vcc_i2c)) {
        ...
        HYN_ERROR("Regulator get failed vcc_i2c ret=%d\n", PTR_ERR(ts_data->vcc_i2c));
    } else {
        HYN_ERROR("Regulator get vcc_i2c success\n");
    }
	if (!IS_ERR_OR_NULL(ts_data->vcc_i2c)){
		ret = regulator_set_voltage(ts_data->vcc_i2c, 3300000,3300000);
		if (ret) {
			regulator_put(ts_data->vcc_i2c);
			ts_data->vcc_i2c = NULL;
			HYN_ERROR("ktf Regulator set 2.8V failed vcc_i2c ret=%d\n", ret);
			//return ret;
		} else {
			HYN_ERROR("ktf Regulator set 2.8V success\n");
			
			ret = regulator_enable(ts_data->vcc_i2c);
			if (ret) {
				HYN_ERROR("GTP enable vcc_i2c failed\n");
				//return ret;
			} else {
				HYN_ERROR("GTP enable vcc_i2c success\n");
			}
		}
	}
	mdelay(40);
...
#define CST6XX_BIN_SIZE    (24*1024 + 28)
{
	...
	if (ts_data->p_hynitron_upgrade_firmware){
		switch(ts_data->config_chip_series){
			...
			case HYN_CHIP_CST6XX:
				pBuf = &ts_data->p_hynitron_upgrade_firmware[CST6XX_BIN_SIZE-16];
				fw_version = pBuf[7];
				fw_version <<= 8;
				fw_version |= pBuf[6];
				fw_version <<= 8;
			...
			case HYN_CHIP_CST3XX: 
				...
				pBuf = &ts_data->p_hynitron_upgrade_firmware[CST3XX_BIN_SIZE-16];
				fw_version = pBuf[7];
				fw_version <<= 8;
				fw_version |= pBuf[6];
				fw_version <<= 8;
	...
	sprintf(current_tp_info.chip,"0x%04x(type)", ts_data->chip_ic_type);
	sprintf(current_tp_info.id,"0x%04x(project)\n  fw:0x%x",ts_data->chip_ic_project_id,ts_data->chip_ic_fw_version);
	sprintf(current_tp_info.vendor,"Hynitron");
	sprintf(current_tp_info.more,"module:%d hfw:0x%x",ts_data->chip_ic_module_id,fw_version);
}
#endif
//prize added by huarui, fts support, 20190111-end
...
err_end:
	if (!IS_ERR(ts_data->vcc_i2c)) {
		if (regulator_enable(ts_data->vcc_i2c)){
			HYN_ERROR("GTP enable vcc_i2c failed\n");
		}
		regulator_put(ts_data->vcc_i2c);
	};
...
#endif
#if defined(CONFIG_FB)
	fb_unregister_client(&hyn_ts_data->fb_notif);
#endif
   //if point is null,may cause dump
	...
	if(hyn_ts_data->input_dev!=NULL){
    	input_unregister_device(hyn_ts_data->input_dev);
		input_free_device(hyn_ts_data->input_dev);
		hyn_ts_data->input_dev = NULL;
	}
	if(hyn_ts_data->hyn_workqueue!=NULL){
    	destroy_workqueue(hyn_ts_data->hyn_workqueue);
		hyn_ts_data->hyn_workqueue = NULL;
	}
