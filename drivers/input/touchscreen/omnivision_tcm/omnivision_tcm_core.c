...
/*prize add by liaoxingen 202030325 end */
...
static int ovt_tcm_probe(struct platform_device *pdev)
{
	...
	//retval = sysfs_create_link(NULL, tcm_hcd->sysfs_dir, "touchscreen");
	//if (retval < 0) {
	//	LOGE(tcm_hcd->pdev->dev.parent, "Failed to create link!");
	//	return -ENOMEM;
	//}
...
err_sysfs_create_dynamic_config_file:
	//sysfs_remove_link(NULL, "touchscreen");//prize add by huarui , tp suspend, 20210408
	for (idx--; idx >= 0; idx--) {
...
#ifdef CONFIG_PM
static __maybe_unused const struct dev_pm_ops ovt_tcm_dev_pm_ops = {
//#ifndef CONFIG_FB
...
static struct platform_driver ovt_tcm_driver = {
	.driver = {
		...
		.owner = THIS_MODULE,
#ifdef CONFIG_PM
		//.pm = &ovt_tcm_dev_pm_ops,	//prize modified by huarui
#endif
	},
	.probe = ovt_tcm_probe,
