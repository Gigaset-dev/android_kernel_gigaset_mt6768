...
/* Magic number of dealy in us */
#define FSA4480_DUMMY_REG_DELAY_US        0xFF
...
static const struct fsa4480_reg_val fsa_reg_i2c_defaults[] = {
	...
	{FSA4480_SWITCH_SETTINGS, 0x80},
	{FSA4480_SWITCH_CONTROL, 0x18},
	{FSA4480_DUMMY_REG_DELAY_US, 50},
...
static void fsa4480_update_reg_defaults(struct regmap *regmap)
{
	...
	for (i = 0; i < ARRAY_SIZE(fsa_reg_i2c_defaults); i++) {
		if (FSA4480_DUMMY_REG_DELAY_US == fsa_reg_i2c_defaults[i].reg) {
			usleep_range(fsa_reg_i2c_defaults[i].val, fsa_reg_i2c_defaults[i].val + 5);
			continue;
		}
