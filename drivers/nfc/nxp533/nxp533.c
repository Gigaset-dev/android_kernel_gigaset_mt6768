...
//prize add by zhaopengge 20201029 start
#ifdef CONFIG_MTK_CLKBUF_NFC
extern int _clk_buf_ctrl_internal(enum clk_buf_id id,enum cmd_type cmd);
#endif
//prize add by zhaopengge 20201029 end

//prize add by huarui 20201014 start
#if defined(CONFIG_HISENSE_PRODUCT_DEVINFO)
#include <hisense/productinfo.h>
#endif
//prize add by huarui 20201014 end
			       ...
			       size_t count, loff_t *offset)
{
	...
	//printk(" %s : mutex_lock.\n", __func__);
	if (!__gpio_get_value(pn547_dev->irq_gpio)) {
		...
		//printk("pn547 read2  __gpio_get_value(pn547_dev->irq_gpio)=%d\n", __gpio_get_value(pn547_dev->irq_gpio));
		ret = wait_event_interruptible(pn547_dev->read_wq, __gpio_get_value(pn547_dev->irq_gpio));
		//printk("pn547 read2  __gpio_get_value(pn547_dev->irq_gpio)=%d\n", __gpio_get_value(pn547_dev->irq_gpio));
	...
	switch (cmd)
	{
		case PN547_SET_PWR:
			if (arg == 2) {
				...
				#ifdef CONFIG_MTK_CLKBUF_NFC
					_clk_buf_ctrl_internal(CLK_BUF_NFC,CLK_BUF_ON);
				#endif
				...
				/* power on */
				pn547_enable_irq(pn547_dev);
				printk("pn547 %s power on start...\n", __func__);
				ret = pn547_platform_pinctrl_select(gpctrl, st_dwn_l);
				ret = pn547_platform_pinctrl_select(gpctrl, st_ven_h);
				msleep(10);
				printk("pn547 %s power on end...\n", __func__);
			} else  if (arg == 0) {
				//clk_buf_ctrl(CLK_BUF_NFC, 0);
				#ifdef CONFIG_MTK_CLKBUF_NFC
					_clk_buf_ctrl_internal(CLK_BUF_NFC,CLK_BUF_OFF);
				#endif
				/* power off */
				pn547_disable_irq(pn547_dev);
			...
			const struct i2c_device_id *id)
{
	...
	pn547_dev->pn547_device.name = "pn553";
	...
	*/
	...
	if (node) {
		...
		if (ret < 0) {
			printk("Unable to request nfc irq_gpio\n");
		}
		else
			printk("request nfc irq_gpio success!!!\n");
		...
		ret = request_irq(client->irq, pn547_dev_irq_handler,IRQ_TYPE_LEVEL_HIGH, "nfc_eint_as_int", pn547_dev);
...
//drv add by liuxuhui for nxp nfc end -- 20230912
	...
	//prize add by huarui 20201014 start
	#if defined(CONFIG_HISENSE_PRODUCT_DEVINFO)
	productinfo_register(PRODUCTINFO_NFC_ID, "NQ310", "NXP");
	#endif
	//prize add by huarui 20201014 end
