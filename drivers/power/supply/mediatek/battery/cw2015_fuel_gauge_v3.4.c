...
static int cw_get_capacity(struct cw_battery *cw_bat)
{
	...
	if ((cw_bat->charger_mode > 0) && (cw_capacity >= 95) && (cw_capacity <= cw_bat->capacity)) {
        ...
        cw_printk(1,"Chaman join no charge full charging_loop [%d]  max count[%d]\n",charging_loop,BATTERY_UP_MAX_CHANGE / queue_delayed_work_time);		
                ...
                union power_supply_propval *val)
{
    ...
    */        
    ...
    case POWER_SUPPLY_PROP_VOLTAGE_NOW:
            val->intval = cw_bat->voltage;   //cw_bat->voltage * 1000  prize-huangjiwu-8804-ma-out
            break;
